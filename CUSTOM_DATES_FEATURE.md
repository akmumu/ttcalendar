...
### 4. 自动归档
- 不重复日期会保存完整的年、月、日，日期过去后自动进入“已归档”
- 每年重复的生日、纪念日等不会归档，会按下一次发生时间排序
- 已归档日期仍保留在原年份的日历中，沿用原有工作/生活样式
- 首页只展示即将到来的日期，完整管理页可切换查看和编辑已归档日期

### 5. 数据持久化
- 使用 App Group 共享存储 (group.akmumu.ttcalendar)
- 主 App 和小组件共享数据
- 支持每年重复选项
- 兼容旧版本中未保存年份的一次性日期，首次读取时迁移到当前年份
...
6. 一次性日期不会在后续年份重复出现；到期后可在“已归档”中继续管理
