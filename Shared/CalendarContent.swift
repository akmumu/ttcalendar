...
struct NextSpecialDay: Hashable {
    ...
    let dateText: String
    let customDateCategory: CustomSpecialDate.DateCategory?
...
struct UpcomingHighlights: Hashable {
    ...
    let nextCustomSpecialDay: NextSpecialDay?
}
...
enum CalendarContent {
    static func month(containing date: Date = Date(), today: Date = Date()) -> CalendarMonth {
        ...
        let customDates = CustomSpecialDateStore.load()
        ...
        let days = (0..<42).compactMap { offset -> CalendarDay? in
            ...
            let customDate = CustomSpecialDateStore.customDate(
                on: normalizedDate,
                among: customDates,
                calendar: calendar
            )
    ...
    static func nextFestival(after date: Date = Date()) -> NextSpecialDay? {
        nextDay(after: date) { candidateDate, lunar, calendar in
            ...
            return uniqueMarkerTexts([holidayDisplayName] + festivalReminderNames(from: markers)).first
        }
    }
    ...
    static func upcomingHighlights(after date: Date = Date(), includesFestivalAndSolarTerm: Bool = true) -> UpcomingHighlights {
        ...
        var nextCustomSpecialDay: NextSpecialDay?
        let customDates = CustomSpecialDateStore.load()
        ...
        for offset in 0...370 {
            ...
            if nextSpecialDay == nil,
               let name = uniqueMarkerTexts(allNames).first {
                nextSpecialDay = NextSpecialDay(
                    ...
                    customDateCategory: customDate?.category
                )
            }

            if includesFestivalAndSolarTerm,
               nextFestival == nil,
               let name = uniqueMarkerTexts([holidayDisplayName] + festivalReminderNames(from: markers)).first {
                nextFestival = NextSpecialDay(
                    date: candidateDate,
                    name: name,
                    daysRemaining: offset,
                    dateText: dateText(for: candidateDate, calendar: calendar),
                    customDateCategory: nil
                )
            }

            if includesFestivalAndSolarTerm, nextCustomSpecialDay == nil, let customDate {
                nextCustomSpecialDay = NextSpecialDay(
                    date: candidateDate,
            ...
            if nextSpecialDay != nil,
               (!includesFestivalAndSolarTerm || (nextFestival != nil && nextSolarTerm != nil && nextCustomSpecialDay != nil)) {
                break
            }
        }
        ...
        return UpcomingHighlights(
            ...
            nextCustomSpecialDay: nextCustomSpecialDay
        )
    }
    ...
    private static func festivalReminderNames(from markers: [DayMarker]) -> [String] {
        markers
            .filter { !SolarTerm.isTermName($0.name) }
            .map(\.name)
    }
    ...
    private static func visibleHolidayDisplayName(_ displayName: String?, badgeText: String?, markers: [DayMarker]) -> String? {
        ...
        let matchingMarkers = markers.filter { normalizedMarkerText($0.name) == normalizedDisplayName }
        if !matchingMarkers.isEmpty && matchingMarkers.allSatisfy({ SolarTerm.isTermName($0.name) }) {
            return nil
        }
        ...
        return matchingMarkers.contains(where: \.isFestival) ? displayName : nil
    }
...
enum SolarTerm {
    ...
    static func isTermName(_ name: String) -> Bool {
